...
function display() {
    ...
    if (isPuzzleBuilt == true) {
        alert("refresh the Page to rebuild");
        return;
    }

    if (document.getElementById("number").value == "s") {
    ...
    for (let i = 0; i < row; i++) {
        ...
        for (let j = 0; j < col; j++) {
            if (idCount == (num * num) - 1) {
                ...
                cells.push({ 'xPosition': xPosition, 'yPosition': yPosition, 'flag': 0 });
            }
...
function move(ev) {
    ...
    console.log(cells[dropElement].xPosition)
    let flag = false;
    //handling right moves
    if (dragElement % num < (num - 1) && cells[dropElement].flag == -1) {

        if (cells[dragElement + 1].flag == -1) {
    ...
    //handling down moves
    if (dragElement < ((num * num) - num) && cells[dropElement].flag == -1) {
        if (cells[dragElement + num].flag == -1) {
            document.getElementById("img" + dragElement).style.backgroundImage = "";
            document.getElementById("img" + dragElement).style.backgroundColor = "white";
            document.getElementById("img" + dropElement).style.backgroundImage = "url('./chess.jpg.')";
            document.getElementById("img" + dropElement).style.backgroundPosition = cells[dragElement].x + "% " + cells[dragElement].y + "% ";
    ...
    //handling up moves
    if (dragElement < (num * num) && dragElement > num - 1 && cells[dropElement].flag == -1) {
    ...
    //handling left moves
    if (dragElement % num > 0 && cells[dropElement].flag == -1) {
        if (cells[dragElement - 1].flag == -1) {
            document.getElementById("img" + dragElement).style.backgroundImage = "";
            document.getElementById("img" + dragElement).style.backgroundColor = "white";
            document.getElementById("img" + dropElement).style.backgroundImage = "url('./chess.jpg.')";
