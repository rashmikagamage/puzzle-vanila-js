...
function move(ev) {
    ...
    console.log(cells[dragElement].x)
    ...
    //handling up moves
    if (dragElement < (num * num) && dragElement > num && cells[dropElement].x == -1) {
        if (cells[dragElement + 1].x == -1) {
            temp = cells[dragElement];
            cells[dragElement] = 0;
            cells[dropElement] = temp;
    ...
    //handling left moves
    if (dragElement % num > 1 && cells[dropElement].x == -1 {
            if (cells[dragElement + 1].x == -1 cells[dragElement - 1] == 0) {
                temp = cells[dragElement];
                cells[dragElement] = 0;
                cells[dropElement] = temp;
                moves++;
